...
def network_CAM_CASA_WGAP_v4(cfg):
    ...
    '''
    class CAM_SpatialAttention_CASA_WGAP_v4(backbone):
        ...
        def forward(self, y, _=None, labels=None):
            ...
            masks_ = masks_[:, 1:]
            cls_fg = (masks_.mean(-1) * labels[:, 1:]).sum(-1) / labels[:, 1:].sum(-1)

            # upscale the masks & clean
            masks = self._rescale_and_clean(masks, y, labels[:, 1:])
            mask_logits = x
            # attention loss
            loss_at = torch.sum(attention_map.pow(2), dim=-1)
            return cls, cls_fg, {"cam": masks}, mask_logits, None, None, loss_at
    ...
    return CAM_SpatialAttention_CASA_WGAP_v4