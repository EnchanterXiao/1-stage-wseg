...
def network_factory(cfg):
    ...
    '''
        CAM+SpatialAttention+ASPP
        '''
    class CAM_SpatialAttention_v2(backbone):
    ...
    '''
        CAM+SpatialAttention+ChannelAttention
    '''

    class CAM_SpatialAttention_CA(backbone):
    ...
    class CAM_SpatialAttention_WGAP(backbone):
        ...
        def forward_cls(self, x, attention_map):
            bs, c, h, w = x.size()
            x = torch.reshape(x, (bs, c, h*w))
            cls_vec = torch.mul(x, attention_map)
            cls_vec = torch.sum(cls_vec, dim=-1)
            cls_vec = torch.reshape(cls_vec, (bs, c, 1, 1))
            return self.cls_branch(cls_vec)
        ...
        def forward(self, y, _=None, labels=None):
            ...
            cls = self.forward_cls(x, attention_map)
            logits, masks = self.forward_mask(x, y.size()[-2:])
    ...
    class CAM_SpatialAttention_CA_WGAP(backbone):
    ...
    elif cfg.MODEL == 'CS_v2':
        return CAM_SpatialAttention_v2
    elif cfg.MODEL == 'CS_CA':
        return CAM_SpatialAttention_CA
    elif cfg.MODEL == 'CS_WGAP':
        return CAM_SpatialAttention_WGAP
    elif cfg.MODEL == 'CS_CA_WGAP':
        return CAM_SpatialAttention_CA_WGAP
    else:
