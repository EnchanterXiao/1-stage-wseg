...
class GroupAttention(nn.Module):
    ...
    """
    def __init__(self, input_dim, output_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., ws=1):
        assert ws != 1
        ...
        self.qkv = nn.Linear(self.input_dim, self.output_dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.output_dim, self.output_dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.ws = ws
    ...
    def forward(self, x, H, W):
        ...
        qkv = self.qkv(x).reshape(B, total_groups, -1, 3, self.num_heads, self.output_dim // self.num_heads).permute(3, 0, 1, 4, 2, 5)
        ...
        attn = (attn @ v).transpose(2, 3).reshape(B, h_group, w_group, self.ws, self.ws, self.output_dim)
        x = attn.transpose(2, 3).reshape(B, N, self.output_dim)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x


def network_CAM_CASA_WGAP_tf_v3(cfg):
    ...
    '''
    class CAM_SpatialAttention_CASA_WGAP_tf_v3(backbone):
    ...
    return CAM_SpatialAttention_CASA_WGAP_tf_v3